...
class AbstractPlModel(LightningModule, AbstractDynamicsModel, metaclass=abc.ABCMeta):
    ...
    def predict(self, states: np.ndarray, actions: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        """
        pt_states = torch.Tensor(states, device=self.device)
        pt_actions = torch.Tensor(actions, device=self.device)
        if len(states.shape) == 1:
            pt_states = pt_states.unsqueeze(-1)
        if len(actions.shape) == 1:
            pt_actions = pt_actions.unsqueeze(-1)
        net_in = torch.cat([pt_states, pt_actions], dim=1)
        deltas, infos = self._get_deltas_from_torch(net_in)
        if len(states.shape) == 1:
            deltas = deltas.flatten()
        nxts = states + deltas.numpy()
        return nxts, infos
    ...
    def get_net_out(self, batch: Sequence[torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        Returns: Dictionary of name to tensor.
        """
    ...
    def _get_deltas_from_torch(self, net_in: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        Returns: The deltas for next states and dictionary of info.
        """
