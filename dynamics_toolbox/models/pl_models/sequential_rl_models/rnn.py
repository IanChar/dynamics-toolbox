...
class RNN(AbstractSequentialRlModel):
    ...
    def __init__(
        ...
        self._memory_unit = torch.nn.GRU(encode_dim, hidden_size, num_layers,
                                         device=self.device)
    ...
    def get_net_out(self, batch: Sequence[torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        predictions = []
        hidden = torch.zeros(self._hidden_size, obs.shape[0], self._encoder_dim,
                             device=self.device)
        curr = obs[:, 0, :]
        for t in range(obs.shape[1]):
            ...
            predictions.append(self._dcoder(mem_out.squeeze(0)))
        ...
        return {'prediction': torch.stack(predictions)}
    ...
    def single_sample_output_from_torch(self, net_in: torch.Tensor) -> Tuple[
        ...
        if self._hidden_state is None:
            self._hidden_state = torch.zeros(self._hidden_size, net_in.shape[0],
                                             self._encoder_dim, device=self.device)
