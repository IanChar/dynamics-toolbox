"""
Loss functions for quantile models
...
def cali_loss(model, x, y, q, args):
    """
    ...
    if x is None:
        ...
        q_tensor = q * torch.ones(num_pts).view(-1, 1)
    ...
    if hasattr(args, "sharp_penalty"):
        ...
        if x is None:
            opp_q_tensor = torch.Tensor([1 - q])
            opp_pred_y = model(opp_q_tensor)
        else:
            opp_q_tensor = (1 - q) * torch.ones(num_pts).view(-1, 1)
...
def batch_cali_loss(model, x, y, q_list, args):
    """
    ...
    if args.sharp_penalty is not None:
        ...
        if hasattr(args, "sharp_all") and args.sharp_all:
            ...
            with torch.no_grad():
                ...
                obs_interval_props = torch.mean(
                    within_interval_mat.float(), dim=1
                )
                obs_over_exp = obs_interval_props > exp_interval_props
            sharp_penalty = (
                obs_over_exp.reshape(num_q, -1)
...
def batch_qr_loss(model, x, y, q_list, args):
    """
...
def crps_loss(model, x, y, q_list, args):
    """
...
def interval_loss(model, x, y, q, args):
    """
    ...
    l_rep = lower.view(-1, 1).repeat(1, num_pts).view(-1, 1)
    u_rep = upper.view(-1, 1).repeat(1, num_pts).view(-1, 1)
...
def batch_interval_loss(model, x, y, q_list, args):
    """
