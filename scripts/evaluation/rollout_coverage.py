...
parser.add_argument('--save_dir', type=str, default='coverage_figures')
...
dcovs, covs, sharpnesses, avg_ev, median_ev, best_ev = [[] for _ in range(6)]
amounts = [0.25, 0.5, 0.9]
for h in tqdm(range(args.horizon)):
    currdcov, currcov, currsharp = [[] for _ in range(3)]
    for amt in amounts:
        dcov, cov, sharpness = coverage_and_sharpness_from_samples(
            preds[:, :, h],
            obs[:, h],
            coverage_amount=amt,
        )
        currdcov.append(dcov)
        currcov.append(cov)
        currsharp.append(sharpness)
    dcovs.append(np.array(currdcov))
    covs.append(np.array(currcov))
    sharpnesses.append(np.array(currsharp))
    all_resids = np.array([
...
dcovs, covs, sharpnesses, avg_ev, median_ev, best_ev = [
    np.array(ar) for ar in (dcovs, covs, sharpnesses,
                            avg_ev, median_ev, best_ev)]
...
def plot_cov(cov, sharpness, aev, mev, bev, title, save_path=None, show=False):
    tstep = np.arange(1, len(cov) + 1)
    fig, axs = plt.subplots(1, 3)
    axs[0].plot(tstep, cov[:, 0], color='red')
    axs[0].axhline(0.25, ls='--', color='red')
    axs[0].plot(tstep, cov[:, 1], color='green')
    axs[0].axhline(0.5, ls='--', color='green')
    axs[0].plot(tstep, cov[:, 2], color='blue')
    axs[0].axhline(0.9, ls='--', color='blue')
    axs[0].set_xlabel('Time Step')
    axs[0].set_ylabel('Coverage')
    axs[0].set_ylim([-0.1, 1.1])
    axs[1].plot(tstep, sharpness[:, 0], 'red')
    axs[1].plot(tstep, sharpness[:, 1], 'green')
    axs[1].plot(tstep, sharpness[:, 2], 'blue')
    axs[1].set_xlabel('Time Step')
    axs[1].set_ylabel('Sharpness')
    axs[2].plot(tstep, aev, label='Average')
    axs[2].plot(tstep, mev, label='Median')
    axs[2].plot(tstep, bev, label='Best')
    axs[2].set_xlabel('Time Step')
    axs[2].set_ylabel('1 - EV')
    axs[2].legend()
...
for dim in range(covs.shape[2]):
    plot_cov(covs[..., dim], sharpnesses[..., dim], avg_ev[:, dim],
             median_ev[:, dim], best_ev[:, dim],
             title=f'Dimension {dim + 1}',
             save_path=os.path.join(args.save_dir, f'dim_{dim+1}.png'))
plot_cov(dcovs, np.mean(sharpnesses, axis=-1),
